...
my $PriceListService = WebServiceClient
    ->uri( 'urn://epages.de/WebService/PriceListService/2010/08' )
...
my %Options = (
    ...
    'GlobalValueDiscounts_in' => [
        { Value=>50, Discount=>10 },
        { Value=>200, Discount=>15 }
    ],
    'GlobalValueDiscounts_up' => [
        { Value=>200, Discount=>20 },
        { Value=>500, Discount=>33.33 }
    ],
);
...
my $PriceList_in = {
    ...
    'GlobalValueDiscounts' => $Options{'GlobalValueDiscounts_in'},
};
...
my $PriceList_update = {
    ...
    'GlobalValueDiscounts' => $Options{'GlobalValueDiscounts_up'},
};

my $PriceList_update_empty = {
    'Path'          => $Options{'FullPath'},
    'Name'          => TLocalizedString( $Options{'Name_up'} ),
...
my $PriceList_update_undef = {
    'Path'          => $Options{'FullPath'},
    'Name'          => TLocalizedString( $Options{'Name_up'} ),
    'GlobalValueDiscounts' => undef,
    'CustomerGroups' => undef,
    'Customers' => undef,
};
...
sub testUpdate {
    my( $PriceList ) = @_;
    $PriceList = $PriceList_update unless $PriceList;

    my $ahResults = $PriceListService->update( [$PriceList] )->result;
...
sub deleteIfExists {
    ...
    if ( $ahResults->[0]->{'exists'} ) {
        $ahResults = $PriceListService->delete( [$Options{'FullPath'}] )->result;
        die $ahResults->[0]->{'Error'}->{'Message'} if $ahResults->[0]->{'Error'};
    }
}
...
deleteIfExists();
testCreate();
testUpdate($PriceList_update_empty);
deleteIfExists();
testCreate();
testUpdate($PriceList_update_undef);
testDelete();

...
my $ProductService = WebServiceClient
    ->uri( 'urn://epages.de/WebService/ProductService/2015/01' )
...
my $Product_in = {
    ...
    'EcoParticipationCategory' => '/EcoParticipationCategories/1',
    'ShippingMethods' => [
    ...
    'EnergyLabelSourceFile' => {
            'Value'   => 'energylabel.jpg',
            'FileContent'=> SOAP::Data->type( 'base64' => GetFileContent('../../../TestData/ProductService/label_refrigerators7.jpg')),
    }
    ,
    'EnergyLabelsString' => 'A++,E',
    'ProductDataSheet' => [
        {
          'LanguageCode' => 'de',
          'Value'   => 'G1.pdf',
          'FileContent'=> SOAP::Data->type( 'base64' => GetFileContent('../../../TestData/ProductService/G1.pdf')),
        },
        {
          'LanguageCode' => 'en',
          'Value'   => 'G1_en.pdf',
          'FileContent'=> SOAP::Data->type( 'base64' => GetFileContent('../../../TestData/ProductService/G1_en.pdf')),
        },
    ],
...
my $Product_update = {
    ...
    'EcoParticipationCategory' => '/EcoParticipationCategories/3',
    'ShippingMethods' => [
...
sub testGetInfo {
	...
	#print Data::Dumper::Dumper($ahResults);
    ...
    #special case EcoParticipationName is read only
    if ( $alreadyUpdated ) {
        $LocReference{EcoParticipationCategoryName}{de} = 'Datenverarbeitungsanlagen und Telekommunikation';
        $LocReference{EcoParticipationCategoryName}{en} = 'Computers and telecommunication';
    } else {
        $LocReference{EcoParticipationCategoryName}{de} = "Gro\x{df}e Haushaltsger\x{e4}te";
        $LocReference{EcoParticipationCategoryName}{en} = 'Large appliances';
    }
    ...
    ok( $LocResult{'EcoParticipationCategoryName'}{'en'} eq $LocReference{'EcoParticipationCategoryName'}{'en'}, "ecoparticipation name (en)" );
    ok( $LocResult{'EcoParticipationCategoryName'}{'de'} eq $LocReference{'EcoParticipationCategoryName'}{'de'}, "ecoparticipation name (de)" );
    ok( $hResult->{'EcoParticipationCategory'} eq $hReference->{'EcoParticipationCategory'}, "ecoparticipation category alias" );
    ...
    ok( $hResult->{'EnergyLabelsString'} eq $Product_in->{'EnergyLabelsString'}, "EnergyLabelsString" );
    ok( $hResult->{'EnergyLabelSourceFile'} eq $Product_in->{'EnergyLabelSourceFile'}{'Value'}, "EnergyLabelSourceFile" );
    ok( $LocResult{'ProductDataSheet'}{'en'} eq $LocReference{'ProductDataSheet'}{'en'}, "ProductDataSheet (en)" );
    ok( $LocResult{'ProductDataSheet'}{'de'} eq $LocReference{'ProductDataSheet'}{'de'}, "ProductDataSheet (de)" );
}
...
sub testFindAll {
    ...
    is( scalar @$aResults, 87, "find all result count" );
...
sub testSetPrice {
    ...
    for my $hPriceInfo (@{$hResult->{'ProductPrices'}}) {
        if($hPriceInfo->{'CurrencyID'} eq 'EUR') {
            is($hPriceInfo->{'Price'}, $Price, "price is $Price");
        }
    }
}

...
class ProductService11StubFactoryImpl implements ProductService11StubFactory {
    ...
    @Override
    public Bind_Product_SOAPStub create(WebServiceConfiguration config, ProductService service) {
        log.info("address specified by wsdl: " + service.getport_ProductAddress());
        log.info("using web service Url: " + config.getWebserviceURL());
            try {
                Bind_Product_SOAPStub stub = new Bind_Product_SOAPStub(config.getWebserviceURL(), service);
